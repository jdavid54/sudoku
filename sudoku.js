...
function showForm() {      //crée form et affiche les valeurs de rows
	content='';
	content+='<p id="title"><b>Sudoku grid input</b></p>';
	content+='<p>Enter 0 for unknown numbers</p>';
	content+='<p>Replace them with your solutions!</p>';
...
function fillForm() {     // met à jour de form avec rows[] qui doit etre mis à jour avec sync() à partir de candidates
	for (k=0;k<9;k++) {
...
function updateValues() {  //lit les valeurs dans form et met à jour rows si on rentre une valeur
	for (k=0;k<9;k++) {
		var x = document.getElementById("row"+(k+1));
		rows[k] = x.value;
	}
...
function populate() {
  ...
  for (m=0;m<9;m++){
	for (n=0;n<9;n++){	
		candidates[m][n]='';
		combo[m][n]=rows[m]+'-'+columns[n]+'-'+zones[3*Math.floor(m/3)+Math.floor(n/3)];
...
function makegrid(array) {	// crée la grille avec candidates
	content='';
	content+='<style>td {font-family:arial; border: 1px solid black; font-size:12px; text-align:left; vertical-align:top; width:40px; height:40px;} .found {font-weight:bold;font-size:20px;text-align:center; vertical-align:middle;} div{display:inline; float:left;} p {font-family:arial; font-size:12px;} .grey {background-color:lightgrey;}</style>';
...
function sync(){  // prend les valeurs de candidates pour mettre à jour rows[] et form avec fillForm()
	for (k=0;k<9;k++){
		for (l=0;l<9;l++){
...
function updateAll(){
	populate();
	scan();  // cherche unique ed doublets dans candidates	
	makegrid(candidates);  // redessine grille avec nouvelles valeurs de candidates
	sync();  // met à jour rows[] à partir de candidates	
}
