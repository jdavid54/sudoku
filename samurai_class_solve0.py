...
while not solved:
    ...
    for g,c,t in ((samurai0, ul, 'lr'), (samurai1, ur, 'll'), (samurai3, ll, 'ur'), (samurai4, lr, 'ul')):
        # update corner grid upper left
        grid = paste_grid(g, c, t)
        #print(np.matrix(samurai0))
        # solve grid upper left
        s = solve()
        try :
            next(s)
            print(np.matrix(g))
    ...
    '''
    # update corner grid upper left
    grid = paste_grid(samurai0, ul, 'lr')
    #print(np.matrix(samurai0))
    # solve grid upper left
    s = solve()
    try :
    ...
    # if ok continue with upper right
    grid = paste_grid(samurai1, ur, 'll')
    s = solve()
    try :
        next(s)
        print(np.matrix(samurai1))
        solved2 = True
    except:
        print('no solution1')
        #solved = False
    #grid = save
    # if ok continue withlower left
    grid = paste_grid(samurai3, ll, 'ur')
    s = solve()
    try :
        next(s)
        print(np.matrix(samurai3))
        solved3 = True
    except:
        print('no solution3')
    # if ok continue with lower right
    grid = paste_grid(samurai4, lr, 'ul')
    s = solve()
    try :
        next(s)
        print(np.matrix(samurai4))
        solved4 = True
    except:
        print('no solution4')
    # if ok, problem solved !!!
    solved = solved1 and solved2 and solved3 and solved4
    print(solved)
    if solved : break
    # else next(r) of central
    # and repeat todo until solved or StopIterartion of next(r)
    '''
